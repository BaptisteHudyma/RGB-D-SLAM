...
namespace rgbd_slam {
    namespace tracking {
        ...
        std::pair<vectorxd, matrixd> SharedKalmanFilter::get_new_state(const vectorxd& currentState, const matrixd& stateNoiseCovariance, const vectorxd& newMeasurement, const matrixd& measurementNoiseCovariance) 
        {
            // Get new raw estimate
            const vectorxd& newStateEstimate = _systemDynamics * currentState;
            const matrixd& estimateErrorCovariance = _systemDynamics * stateNoiseCovariance * _systemDynamics.transpose() + _processNoiseCovariance;
            
            // compute Kalman gain
            const matrixd& kalmanGain = estimateErrorCovariance * _outputMatrix.transpose() * (_outputMatrix * estimateErrorCovariance * _outputMatrix.transpose() + measurementNoiseCovariance).inverse();
        ...
        void KalmanFilter::update(const vectorxd& newMeasurement, const matrixd& measurementNoiseCovariance) 
        {
            ...
            const std::pair<vectorxd, matrixd>& res = get_new_state(_stateEstimate, _estimateErrorCovariance, newMeasurement,measurementNoiseCovariance);
        ...
        void KalmanFilter::update(const vectorxd& newMeasurement, const matrixd& measurementNoiseCovariance, const matrixd& systemDynamics)
        {
