...
Local_Map::Local_Map(const Parameters& voParams, Image_Features_Handler* featuresHandler) 
    : voParams(voParams), featuresHandler(featuresHandler)
{
    // compute image bounds
    if (fabs(this->voParams.get_k1()) < 1e-5)
    {
        min_x = 0.0;
        max_x = this->voParams.get_width();
        min_y = 0.0;
        max_y = this->voParams.get_height();
    }
    else
    {
        ...
        kps_mat.at<float>(1, 0) = this->voParams.get_width();
        kps_mat.at<float>(1, 1) = 0.0;
        kps_mat.at<float>(2, 0) = 0.0;
        kps_mat.at<float>(2, 1) = this->voParams.get_height();
        kps_mat.at<float>(3, 0) = this->voParams.get_width();
        kps_mat.at<float>(3, 1) = this->voParams.get_height();
        ...
        vector3_array& outMapPoints, std::vector<int>& outMatchesLeft)
{
    ...
    for (int i = 0; i < this->mapPoints.size(); i++)
    {
        ...
        if (!is_point_visible(this->mapPoints[i].position, cml, this->voParams, proj_pt_left))
        {
        ...
        float d1, d2;
        int match_idx_left = features.find_match_index(proj_pt_left, this->mapPoints[i].descriptor, &d1, &d2);
    ...
    { //search at 2x the tracking radius
        ...
        for (int i = 0; i < this->mapPoints.size(); i++)
        {
            ...
            float d1, d2;
            int match_idx_left = features.find_match_index(projections[i], this->mapPoints[i].descriptor, &d1, &d2);
            matches[i] = match_idx_left;
            if (match_idx_left != -1)
            {
                matches_count++;
    ...
    for (int i = 0; i < matches.size(); i++)
    {
        ...
        this->mapPoints[i].age += 1;
        outMapPoints.push_back(this->mapPoints[i].position);
...
{
    const float inv_fx = 1.0f / this->voParams.get_fx();
    const float inv_fy = 1.0f / this->voParams.get_fy();
    ...
    for (int i = 0, count = features.get_features_count(); i < count; i++)
    {
        ...
        const float x = (u - this->voParams.get_cx()) * z * inv_fx;
        const float y = (v - this->voParams.get_cy()) * z * inv_fy;
        vector4 pt_h;
        pt_h << x, y, z, 1.0;
...
/*void Local_Map::triangulate(const Pose &camPose, Image_Features_Struct& features, Image_Features_Struct& featuresRight, mapPointArray& outPoints)  {
    ...
    for (size_t i = 0, count = matches.size(); i < count; i++)
    {
        ...
        if (not is_point_visible(world_pt, cml, this->voParams, proj_pt_l) or 
                not is_point_visible(world_pt, cmr, this->voParams, proj_pt_r)) {
            continue;
        }
...
void Local_Map::update_with_new_triangulation(const Pose &camPose, Image_Features_Struct& features, bool dont_stage)
{
    ...
    {
        this->mapPoints.insert(this->mapPoints.end(), new_triangulations.begin(), new_triangulations.end());
    }
    else
    {
        this->stagedPoints.insert(this->stagedPoints.end(), new_triangulations.begin(), new_triangulations.end());
    }
}
...
void Local_Map::update_staged_map_points(const Pose &camPose, Image_Features_Struct& features)
{
    ...
    for (int i = 0, count = this->stagedPoints.size(); i < count; i++)
    {
        mapPoint *mp = &(this->stagedPoints[i]);
        const vector3 world_pt = mp->position;
        vector2 proj_pt_left;
        float d1, d2;
        int match_idx_left = -1;

        if (not is_point_visible(world_pt, cml, this->voParams, proj_pt_left) or
        ...
        if (mp->counter == this->voParams.get_staged_threshold() or 
                get_map_size() < LVT_N_MAP_POINTS) {
            this->mapPoints.push_back(this->stagedPoints[i]);
            points_to_be_deleted.insert(mp);
            n_upgraded_points++;
        }
    }

    this->stagedPoints.erase(std::remove_if(this->stagedPoints.begin(), this->stagedPoints.end(),
                [&points_to_be_deleted](mapPoint &pt) { return points_to_be_deleted.count(&pt) != 0; }),
            this->stagedPoints.end());
}
...
{
    const int th = this->voParams.get_untracked_threshold();
    mapPointArray cleaned_map_points;
    cleaned_map_points.reserve(this->mapPoints.size());
    for (int i = 0; i < this->mapPoints.size(); i++)
    {
        if (this->mapPoints[i].counter >= th)
        {
            if (this->mapPoints[i].match_idx >= 0)
            {
                features.mark_as_matched(this->mapPoints[i].match_idx, false);
            }
        }
        else
        {
            cleaned_map_points.push_back(this->mapPoints[i]);
        }
    }
    cleaned_map_points.swap(this->mapPoints);
}
