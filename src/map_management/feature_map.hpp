...
namespace rgbd_slam {
    namespace map_management {
        ...
        class IMapFeature
        {
            public:
            IMapFeature()
                : _failedTrackingCount(0), _successivMatchedCount(0), _id(++_idAllocator), _matchIndex(FIRST_DETECTION_INDEX)
            {};
            IMapFeature(const size_t id)
                : _failedTrackingCount(0), _successivMatchedCount(0), _id(id), _matchIndex(FIRST_DETECTION_INDEX)
            {};
             ...
             * \param[in] worldToCamera The matrix to convert from map space to camera space. This should be an estimate of the position from which detectedFeatures were observed
             ...
             * \param[in] cameraToWorld The matrix to transform from the camera pose where matchedFeature was detected to world coordinates
             */
            void update_matched(const DetectedFeatureType& matchedFeature, const matrix33& poseCovariance, const cameraToWorldMatrix& cameraToWorld)
            {
             ...
             */
            virtual bool add_to_tracked(const worldToCameraMatrix& worldToCamera, TrackedFeaturesObject& trackedFeatures, const uint dropChance = 1000) const = 0;
        ...
        protected:
            virtual bool update_with_match(const DetectedFeatureType& matchedFeature, const matrix33& poseCovariance, const cameraToWorldMatrix& cameraToWorld) = 0;
        ...
        template<class DetectedFeatureType>
        class IStagedMapFeature
        {
            public:
            //IStagedMapFeature(const matrix33& poseCovariance, const cameraToWorldMatrix& cameraToWorld, const DetectedFeatureType& detectedFeature);
            virtual bool should_remove_from_staged() const = 0;
            virtual bool should_add_to_local_map() const = 0;
        };

        template<class StagedMapFeature>
        class ILocalMapFeature
        {
            public:
         ...
         */
        template<class MapFeatureType, class StagedFeatureType, class DetectedFeaturesObject, class DetectedFeatureType, class FeatureMatchType, class TrackedFeaturesObject>
        class Feature_Map
        {
            private:
             ...
             * \param[in] addAllFeatures If true, will add all detected features to the staged map. If false, will only add the unmatched features to staged map
             */
            ...
            void update_local_map(const cameraToWorldMatrix& cameraToWorld, const matrix33& poseCovariance, const DetectedFeaturesObject& detectedFeatureObject)
            {
            ...
            void update_staged_map(const cameraToWorldMatrix& cameraToWorld, const matrix33& poseCovariance, const DetectedFeaturesObject& detectedFeatureObject)
            {
