...
class LSD
{
  public:
    /**
     * Create a LineSegmentDetectorImpl object. Specifying scale, number of subdivisions for the image, should the lines
     * be refined and other constants as follows:
     *
     ...
     * approximations. LSD_REFINE_ADV  - Advanced refinement. Number of false alarms is calculated, lines are refined
     * through increase of precision, decrement in size, etc.
     ...
     */
    LSD(int _refine = LSD_REFINE_STD,
        double _scale = 0.8,
    ...
    void detect(const Mat& imageSrc,
  ...
  private:
    ...
    /**
     * Try some rectangles variations to improve NFA value. Only if the rectangle is not meaningful (i.e., log_nfa <=
     * log_eps).
