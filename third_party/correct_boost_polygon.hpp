...
template<typename C, typename T> static inline void result_combine(C& result, T&& new_element)
{
...
template<typename C, typename T> static inline void result_combine_multiple(C& result, T& new_elements)
{
...
struct compare_pseudo_vertice_key
{
    bool operator()(pseudo_vertice_key const& a, pseudo_vertice_key const& b) const
    {
        ...
        if (a.scale > b.scale)
            return false;
        if (a.index_2 > b.index_2)
            return true;
        if (a.index_2 < b.index_2)
            return false;
        if (a.reroute && !b.reroute)
            return true;
        if (!a.reroute && b.reroute)
            return false;
        return false;
...
template<typename point_t = boost::geometry::model::d2::point_xy<double>> struct pseudo_vertice
{
    point_t p;
    pseudo_vertice_key link;

    pseudo_vertice(point_t p, pseudo_vertice_key link = pseudo_vertice_key()) : p(p), link(link) {}
};
        ...
        std::set<pseudo_vertice_key, compare_pseudo_vertice_key>& start_keys)
{
    ...
    typedef boost::geometry::detail::overlay::turn_info<point_t> turn_info;
...
static inline double correct_orientation(ring_t& ring, boost::geometry::order_selector order)
{
    ...
    bool should_reverse = (order == boost::geometry::clockwise && area < 0) ||
...
template<typename point_t = boost::geometry::model::d2::point_xy<double>> struct compare_point_less
{
...
static inline std::vector<std::pair<ring_t, double>> dissolve_generate_rings(
        ...
        boost::geometry::order_selector order,
        double remove_spike_min_area = 0.0)
{
...
static inline std::vector<std::pair<ring_t, double>> correct(ring_t const& ring,
                                                             boost::geometry::order_selector order,
                                                             double remove_spike_min_area = 0.0)
{
...
struct fill_non_zero_winding
{
    ...
    {
        auto compare = [](std::pair<multi_polygon_t, double> const& a, std::pair<multi_polygon_t, double> const& b) {
...
template<typename fill_function_t,
         ...
         typename ring_t = boost::geometry::model::ring<point_t>,
         typename multi_polygon_t = boost::geometry::model::multi_polygon<polygon_t>>
static inline void correct(polygon_t const& input,
                           multi_polygon_t& output,
                           double remove_spike_min_area,
                           fill_function_t const& fill,
                           combine_function_t const& combine,
                           difference_function_t const& difference)
{
...
template<typename fill_function_t,
         ...
         typename ring_t = boost::geometry::model::ring<point_t>,
         typename multi_polygon_t = boost::geometry::model::multi_polygon<polygon_t>>
static inline void correct(multi_polygon_t const& input,
                           multi_polygon_t& output,
                           double remove_spike_min_area,
                           fill_function_t const& fill,
                           combine_function_t const& combine,
...
template<typename point_t = boost::geometry::model::d2::point_xy<double>,
         ...
         typename ring_t = boost::geometry::model::ring<point_t>,
         typename multi_polygon_t = boost::geometry::model::multi_polygon<polygon_t>>
static inline void correct_odd_even(multi_polygon_t const& input,
                                    multi_polygon_t& output,
                                    double remove_spike_min_area = 0.0)
{
    impl::correct(
