...
LSD::LSD(int _refine,
    ...
    N_BINS(_n_bins)
{
    assert(_scale > 0 and _sigma_scale > 0 and _quant >= 0 and _ang_th > 0 and _ang_th < 180 and _density_th >= 0 and
           _density_th < 1 and _n_bins > 0);
}

void LSD::detect(const cv::Mat& imageSrc,
                 std::vector<line_descriptor::KeyLine>& keylines,
                 int numOctaves,
                 int scale,
                 const Mat& mask)
{
    ...
    for (int octaveIdx = 0; octaveIdx < (int)lines_lsd.size(); octaveIdx++)
    {
        ...
        for (int k = 0; k < (int)lines_lsd[octaveIdx].size(); k++)
        {
            ...
            LineIterator li(
                    gaussianPyrs[octaveIdx], Point2f(extremes[0], extremes[1]), Point2f(extremes[2], extremes[3]));
...
void LSD::detect(InputArray _image, OutputArray _lines, OutputArray _width, OutputArray _prec, OutputArray _nfa)
{
               ...
               std::vector<double>& nfas)
{
    ...
    const size_t min_reg_size =
            size_t(-LOG_NT / log10(p)); // minimal number of points in region that can give a meaningful event
...
void LSD::region2rect(
        const std::vector<RegionPoint>& reg, const double reg_angle, const double prec, const double p, rect& rec) const
{
...
double LSD::get_theta(const std::vector<RegionPoint>& reg,
                      const double& x,
                      const double& y,
...
bool LSD::refine(std::vector<RegionPoint>& reg,
                 double reg_angle,
                 const double prec,
                 double p,
                 rect& rec,
...
bool LSD::reduce_region_radius(std::vector<RegionPoint>& reg,
                               double reg_angle,
                               const double prec,
                               double p,
                               rect& rec,
